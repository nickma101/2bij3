...
library(data.table)
library(glue)
library(stringr)## First, set the date for article retrieval. For automation purposes, this is set to
...
conn = amcat.connect("https://vu.amcat.nl")## Since amcat.getobjects() wasn't working, this pulls all articles in project 2, articleset 1340, then filters for
...
print(glue("Got {nrow(d_amcat)} rows"))

d_amcat_today = filter(d_amcat, date == scrape_date & sapply(strsplit(d_amcat$title, " "), length) > 2)
d_amcat_today = d_amcat_today[!duplicated(d_amcat_today$title),]## Create a corpus from the article titles, tokenize this and create a dfm weighted by tf-idf.
c_amcat = corpus(d_amcat_today, text_field="text")
tokenized_amcat = tokens(c_amcat, remove_punct = T)
dfm_amcat = dfm(tokenized_amcat)
dfm_amcat_weighted = dfm_tfidf(dfm_amcat, scheme_tf = "prop")## Compute the similarity between titles using cosine similarity, discard all similarities under
...
df_amcat_simil[df_amcat_simil<0.4]=0## Create a dataframe of similar texts for each title, to get the required output format.
df_clustered = as.data.frame(matrix(nrow = length(df_amcat_simil$text1), ncol = 1))
rownames(df_clustered) = rownames(df_amcat_simil)
colnames(df_clustered) = c("similar_texts")for(i in 1:length(df_clustered$similar_texts)){
  simil_list = c()
...
}df_clustered = filter(df_clustered, !is.na(similar_texts))## Add two columns to the amcat dataframe for intensity and negativity, respectively.
## Base these columns on word listsintensifyers = read.table('lijst_intrinsieke_intensiveerders.txt')negatives = read.csv2('polarityLexiconDutchVU.txt', header = F, col.names = c("word", "type", "posneg"))
negatives = negatives %>%
  filter(posneg == 'negative') %>%
  select(word)sent_dict = dictionary(list(intensity = intensifyers$V1, negativity = negatives$word))
c_titles = corpus(d_amcat_today, text_field="title")
tokenized_titles = tokens(c_titles, remove_punct = T)
dfm_titles = dfm(tokenized_titles)dict_dfm = dfm_lookup(dfm_titles, sent_dict)
df_sent = as.data.frame(as.matrix(dict_dfm))## Merge the clustered dataframe with the intensity/negativity dataframe and put everything in the right format.
...
df_texts$titles = paste0("text", df_texts$titles)df_output = merge(df_clustered, df_sent)
df_output = merge(df_output, df_texts)
df_output = df_output %>%
  unnest(cols=c(similar_texts)) %>%
  filter(titles != similar_texts & similar_texts %in% titles)
df_output[,"cluster_id"] = NAid_list = c()
cluster_no = 0
...
df_output = df_output %>%
  ...
  nest(similar_texts = similar_texts)for (i in 1:length(df_output$similar_texts)) {
  x = as.data.frame(df_output$similar_texts[i])
  df_output$similar_texts[i] = list(x[,1])
}df_output = merge(df_output, d_amcat_today, by.x = "text", by.y = 'title')df_output = df_output %>%
  select(publisher, date, url, title = text, text = text.y, intensity, negativity, cluster_id, similar_texts) %>%
  add_rownames(var = "id") %>%
  add_column(section = NA)## Write the final dataframe to a json file for use in 2bij3
